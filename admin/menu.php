...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'update') {
        ...
        if ($id > 0) {
            try {
                $stmt = $pdo->prepare('SELECT img FROM menu WHERE id = ? LIMIT 1');
                $stmt->execute([$id]);
...
$items = [];
try {
    $stmt = $pdo->query('SELECT id, name, description, price, img, active, created_at FROM menu ORDER BY id DESC');
      ...
      <section class="admin-card" id="addForm" aria-labelledby="addTitle">
        ...
        <form method="post" enctype="multipart/form-data">
          ...
          <div class="admin-field">
            ...
            <input name="img" value="<?php echo htmlspecialchars($editItem['img'] ?? '', ENT_QUOTES); ?>" placeholder="https://... or /assets/menu/...">
